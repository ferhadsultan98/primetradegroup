...
const BookCatalog = () => {
  ...
  const handlePageClick = (spreadIndex) => {
    if (spreadIndex % 2 !== 0) spreadIndex--;
      ...
      <div className="pagination">
        <div className="pageIndicators">
          {pages.map((_, index) => (
            <button
              key={index}
              className={`pageIndicator ${index === currentSpread || index === currentSpread + 1 ? 'active' : ''}`}
        ...
        <div className="pageInfo">
          <span className="currentPageText">
            Səhifə {currentSpread + 1}-{currentSpread + 2} / {pages.length}
          </span>
        </div>
      </div>
    </div>
