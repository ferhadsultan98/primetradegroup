...
const Services = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const isInView = servicesRect.top <= window.innerHeight && servicesRect.bottom >= 0;

      if (isInView) {
        const scrollDirection = currentScrollY > lastScrollY ? 'down' : 'up';
        const scrollAmount = Math.abs(currentScrollY - lastScrollY);
        const servicesGrid = servicesSection.querySelector('.servicesGrid');

        if (scrollDirection === 'down') {
          servicesGrid.scrollLeft += scrollAmount * 0.5; // Adjust multiplier for speed
          servicesGrid.querySelectorAll('.serviceCard').forEach((card) => {
  ...
  return (
    <div className="servicesContainer">
      <Helmet>
        <title>SERVICES - PRIME TRADE GROUP MMC</title>
        <meta 
          name="description" 
      ...
      <div className="containerWrapper">
        ...
        <div className="servicesSection" ref={servicesRef}>
